...
{
    private readonly List<string> _validComponentTypes = new()
    {
        "page", "card", "form", "text", "number", "select", "autocomplete", "textarea",
        "date", "datetime", "time", "checkbox", "switch", "radio", "slider", "rating",
        "file", "button", "iconButton", "fab", "table", "dataGrid", "list", "tabs", "stepper",
        "expansion", "dialog", "snackbar", "progress", "chart", "markdown", "grid", "stack",
        "paper", "divider", "textDisplay", "avatar", "chip", "badge", "tooltip", "skeleton",
        "appBar", "drawer", "breadcrumb", "pagination", "tree", "timeline", "carousel",
        "colorPicker", "richText", "jsonEditor", "codeBlock", "kanban", "calendar", "map", "iframe", "custom"
    };
    ...
    private void ValidateComponents(List<DslComponent> components, List<string> errors, List<string> warnings)
    {
        foreach (var component in components)
        {
            ...
            if (!_validComponentTypes.Contains(component.Type))
                warnings.Add($"未识别的组件类型: {component.Type} (Id: {component.Id})");
    ...
    private void ValidateExpressions(DslPage dsl, List<string> errors)
    {
        foreach (var component in dsl.GetAllComponents())
        {
            ...
            {
                try { new Jint.Engine().Evaluate(component.VisibleIf); } catch (Exception ex)
                { errors.Add($"VisibleIf 表达式语法错误 (Id: {component.Id}): {ex.Message}"); }
            }
            if (!string.IsNullOrWhiteSpace(component.DisabledIf))
            {
                try { new Jint.Engine().Evaluate(component.DisabledIf); } catch (Exception ex)
