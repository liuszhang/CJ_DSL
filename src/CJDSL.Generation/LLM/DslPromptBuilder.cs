...
9. 属性命名随目标平台变化：
   - targetPlatform 为 Web 或 Wpf（MudBlazor）时，组件 props 使用 PascalCase（如 Required, ReadOnly, Variant, Color, Elevation, Class, Placeholder, AdornmentIcon, Lines, MaxLength）；
   - targetPlatform 为 React 时，组件 props 使用 camelCase（如 required, readOnly, variant, color, elevation, className, placeholder, adornmentIcon, lines, maxLength），与 @cj/cjdsl-react 渲染器约定一致；
   - targetPlatform 为 Vue 时，组件 props 使用 kebab-case 或 camelCase（由 @cj/cjdsl-vue 约定）。
";
}
